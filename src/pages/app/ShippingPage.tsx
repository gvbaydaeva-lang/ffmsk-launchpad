...
function shippingDispatcherHint(status: ShippingUiStatus): string {
  ...
  if (status === "cancelled") return "Отгрузка отменена. Действие недоступно.";
  ...
  const completeShipmentAssembly = React.useCallback(
    async (doc: ShipmentDoc) => {
      ...
      if (!Array.isArray(rows) || !shippingGroupAllLinesPacked(rows)) {
        toast.info("Действие недоступно");
        return;
      }
                ...
                <Table className="min-w-[1440px] table-auto">
                  <TableHeader>
                    <TableRow className="border-slate-200 bg-slate-50/90 hover:bg-slate-50/90">
                      ...
                      <TableHead className="h-9 w-[110px] whitespace-nowrap px-3 py-2 text-right text-xs font-semibold text-slate-600">Действия</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {documents.map((doc) => {
                      ...
                      return (
                        <React.Fragment key={doc.id}>
                          <TableRow
                            ...
                            className={cn(
                              "h-10 cursor-pointer border-slate-100 text-xs transition-[background-color,box-shadow] duration-300",
                              isSel ? "bg-slate-50" : "",
                          ...
                          >
                            ...
                            <TableCell className="px-3 py-2 text-right" onClick={(e) => e.stopPropagation()}>
                              <Button
                                ...
                                variant="outline"
                                size="sm"
                                className="h-8 px-3 text-xs font-medium tabular-nums"
                                onClick={() => setSelectedId((p) => (p === doc.id ? null : doc.id))}
                              >
                                ...
                                <div className="space-y-4 border-t border-slate-200 p-4">
                                  ...
                                  {uiStatus === "cancelled" ? (
                                    <div className="rounded-md border border-slate-200 bg-slate-100 px-3 py-2">
                                      <p className="text-xs font-medium text-slate-800">Отгрузка отменена. Действие недоступно.</p>
                                    </div>
                                  ) : uiStatus === "shipped" ? (
